...
class BTC15Data:
    ...
    def latest_trade(self) -> float:
        r = self.client.get(
            f"{self.base}/v1beta3/crypto/us/latest/trades",
    ...
    def orderbook(self) -> tuple[float, float]:
        r = self.client.get(
            f"{self.base}/v1beta3/crypto/us/latest/orderbooks",
            params={"symbols": "BTC/USD"},
        )
        r.raise_for_status()
...
def build_signal(now: datetime | None = None) -> BTC15Signal:
    ...
    if window_rows.empty:
        # The first minute can be incomplete; use the latest known close as proxy.
        reference_open = float(bars.close.iloc[-1])
    else:
        reference_open = float(window_rows.open.iloc[0])
    ...
    # Shrunk drift: recent momentum helps, but is deliberately capped so the
    # model does not extrapolate a one-minute spike through the whole window.
    ...
    # Small microstructure adjustments. These are intentionally modest because
    # the BRTI aggregates multiple venues, while Alpaca is only a proxy feed.
    ...
    within_time = (
        settings.btc15_min_seconds_remaining <= seconds_remaining <= settings.btc15_max_seconds_remaining
    )
    confidence_ok = confidence >= settings.btc15_min_confidence
    spread_ok = spread_bps <= 25
    actionable = bool(within_time and confidence_ok and spread_ok)

    reasons = []
    if not within_time:
        reasons.append("outside preferred entry window")
    if not confidence_ok:
        reasons.append(f"confidence below {settings.btc15_min_confidence:.0f}%")
    if not spread_ok:
        reasons.append("BTC proxy spread too wide")
    if actionable:
        reasons.append("probability, timing and liquidity filters passed")
    ...
    return BTC15Signal(
        ...
        reference_open=round(reference_open, 2),
        current_price=round(price, 2),
        move_from_open_pct=round((price / reference_open - 1) * 100, 4) if reference_open > 0 else 0.0,
        direction=direction,
        probability=round(probability, 4),
        fair_price_cents=round(fair_cents, 1),
        max_buy_price_cents=round(max_buy_cents, 1),
        confidence=round(confidence, 1),
        realized_vol_1m_pct=round(sigma_1m * 100, 4),
        momentum_1m_pct=round(mom1, 4),
        momentum_3m_pct=round(mom3, 4),
